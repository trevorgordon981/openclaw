...
export class MemoryIndexManager implements MemorySearchManager {
  ...
  private readonly requestedProvider: "openai" | "local" | "auto";
  private fallbackFrom?: "openai" | "local";
  ...
  private resolveBatchConfig(): {
    ...
    const enabled = Boolean(batch?.enabled && this.openAi && this.provider.id === "openai");
    return {
      enabled,
  ...
  private async activateFallbackProvider(reason: string): Promise<boolean> {
    ...
    const fallbackFrom = this.provider.id as "openai" | "local";

    const fallbackModel =
      fallback === "openai" ? DEFAULT_OPENAI_EMBEDDING_MODEL : this.settings.model;
